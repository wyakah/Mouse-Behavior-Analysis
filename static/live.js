...
class LiveAnalysisView {
  constructor(host, options={}) {
    this.apiBase=options.apiBase||'/api/batches';this.options=options;
    ...
    host.innerHTML=`
      <div class="live-heading"><div><div class="live-eyebrow"><span class="live-dot"></span><span data-live="label">LIVE ANALYSIS</span></div><h2 data-live="title">Live preview</h2><p data-live="subtitle"></p></div><div class="live-tools"><button data-action="hide" aria-expanded="true">Hide preview</button><button data-action="focus" aria-pressed="false">Focus view ↗</button></div></div>
      <div class="live-layout"><div class="live-main">
        ...
        <div class="live-pass"><div><span data-live="pass-label">Preparing recording</span><b data-live="percent">—</b></div><progress data-live="progress" aria-label="Current processing pass"></progress><small data-live="pass-detail"></small></div>
      </div><aside class="live-queue"><div class="live-queue-heading"><h3>Recording queue</h3><span data-live="batch-count"></span></div><div class="live-queue-list"></div></aside></div>
      <div class="live-footnote"><span data-live="connection" role="status"></span></div>`;
    this.q=name=>host.querySelector(`[data-live="${name}"]`);
    this.stages=options.stereotypy?[['preparing','Prepare'],['scoring','Analyze'],['exporting','Export']]:[['preparing','Prepare'],['localizing','Locate'],['tracking','Track'],['scoring','Score'],['rendering','Review']];
    ...
    this.player.apiBase=this.apiBase;
    if(options.stereotypy)this.host.querySelector('.stream-key').hidden=true;
  ...
  watchRecording(index,expand=true){if(!Number.isInteger(index))return;if(expand)this.setCollapsed(false);const poster=this.host.querySelector('.preview-launch img');poster.src='/api/frame?video='+encodeURIComponent(this.batch.entries[index].video)+'&frame=0'+(this.options.stereotypy?'':'&crop=arena');this.player.select(this.batch.id,index);this.renderPlayback();}
  updateBatch(batch) {
    ...
    this.host.classList.toggle('single-recording',batch.entries.length===1);
    ...
    this.q('title').textContent=running?'Live preview':batch.status==='interrupted'?'Analysis interrupted':batch.status==='complete'?'Ready for review.':'Review needs attention.';
    ...
    if(newBatch||this.player.index===null){this.watchRecording(batch.current_index??0,false);if(newBatch&&running&&batch.entries.length===1)this.setCollapsed(false);}
  ...
  async poll(){
    ...
    try{
      const r=await fetch(`${this.apiBase}/${encodeURIComponent(this.batch.id)}/live?image=0`,{cache:'no-store',signal:controller.signal});if(!r.ok)throw Error('Preview status unavailable');
      ...
      this.q('connection').textContent='';this.renderProgress();
  ...
  renderProgress(){
    ...
    this.q('pass-label').textContent=running?`${this.batch.entries[this.batch.current_index]?.id||''} · ${this.options.stereotypy?(s?.message||titles[stage]||stage):(titles[stage]||stage)}`:this.batch.message;
    ...
    this.q('pass-detail').textContent='';
  }
  renderPlayback(){
    ...
    this.q('empty-title').textContent=this.previewHidden?'Preview hidden':p.state==='unavailable'?'Preview unavailable':'Preparing video';
    this.q('empty-text').textContent=this.previewHidden?'':p.message||'';
