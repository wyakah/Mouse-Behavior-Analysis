...
(async () => {
  const reports = [
    ['/reports/stereotypy-priority/', 'test-report.json', 'Review expanded training & validation →'],
    ['/reports/stereotypy-training/', 'training-report.json', 'Review experimental six-behavior models →'],
  ];
  for (const [base, manifest, title] of reports) {
    try {
      const response = await fetch(base + manifest, {method:'HEAD'});
      if (!response.ok) continue;
      const notice = $('experimental-model-report'), link = notice.querySelector('a');
      link.href = base + 'index.html'; link.textContent = title; notice.hidden = false; break;
    } catch (_) { /* The report is optional on a new installation. */ }
  }
})();
(async()=>{
