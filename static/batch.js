...
function renderJob(b){
 ...
 for(const e of completed){
  ...
  if(e.summary){
   const s=e.summary,fmt=n=>Number.isFinite(n)?n.toFixed(2)+' s':'—';
   const grid=el('div',null,'result-grid social-metrics');
   for(const metric of threeChamberMetrics(s)){const cell=el('div',metric.label);cell.append(el('b',metric.value));grid.append(cell);}c.append(grid);
