...
const validSampleIds=()=>draft.entries.every(e=>String(e.id||'').trim()&&['left','right'].includes(e.stranger_side??e.config?.stranger_side??e.config?.target_side))&&new Set(draft.entries.map(e=>String(e.id||'').trim())).size===draft.entries.length;
...
function renderLists(){
 ...
 $('#analysis-help').textContent=busy?'Open Results to follow progress.':!validSampleIds()?'Enter unique mouse IDs and select each stranger position.':count===total&&total?'Ready. Tracking and scoring will run automatically.':'Confirm the regions in every recording to continue.';
...
function renderJob(b){
 ...
 for(const e of completed){
  ...
  const c=el('article',null,'panel result'),heading=el('div',null,'result-heading');heading.append(el('h2','Recording '+e.id),el('span',e.status));c.dataset.sex=e.sex||'unknown';c.dataset.search=[e.id,e.sex,e.genotype].filter(Boolean).join(' ').toLowerCase();c.append(heading,el('p',[e.sex&&e.sex!=='unknown'?e.sex:'',e.genotype].filter(Boolean).join(' · '),'sample-meta'));const source=el('details',null,'recording-source');source.append(el('summary','Source recording'),el('p',e.video.split('/').pop()));c.append(source);
  if(e.summary){
   const s=e.summary,fmt=n=>Number.isFinite(n)?n.toFixed(2)+' s':'—',table=el('table',null,'measurements'),head=el('thead'),hr=el('tr');
   const roleSide=s.stranger_side||s.target_side,chamberLabel=side=>side[0].toUpperCase()+side.slice(1)+(['left','right'].includes(roleSide)&&side!=='center'?` (${side===roleSide?'Stranger':'Object'})`:'');
   for(const t of ['Time',...['left','center','right'].map(chamberLabel)]){const th=el('th',t);th.scope='col';hr.append(th);}head.append(hr);table.append(head);const body=el('tbody');
   ...
   const side=s.stranger_side||s.target_side,total=s.total_chamber_seconds??(s.left_chamber_seconds+s.center_chamber_seconds+s.right_chamber_seconds),pct=s.stranger_interaction_percent??(['left','right'].includes(side)&&s.nose_scoreable_fraction>0&&total>0?100*s[side+'_nose_seconds']/total:null);
   c.append(el('p','Stranger mouse: '+(['left','right'].includes(side)?side:'Not set'),'sample-meta'),el('h3','Stranger Interaction %: '+(Number.isFinite(pct)?pct.toFixed(2)+'%':'—')));
   ...
   const preview=el('details',null,'review-video'),summary=el('summary'),poster=el('img');poster.alt='';poster.loading='lazy';poster.src='/api/frame?video='+encodeURIComponent(e.video)+'&frame=0&crop=arena';summary.append(poster,el('span','▶  Watch annotated review'));const video=el('video');video.controls=true;video.preload='none';video.setAttribute('aria-label',`Annotated review for ${e.id}`);preview.append(summary,video);preview.ontoggle=()=>{if(preview.open){document.querySelectorAll('.review-video').forEach(p=>{if(p!==preview)p.open=false;});liveView.setCollapsed(true);if(!video.getAttribute('src'))video.src=`/outputs/${e.run_id}/review.mp4`;video.play().catch(()=>{});}else video.pause();};c.append(preview);
   ...
   const details=el('details');details.append(el('summary','Detailed exports'));
