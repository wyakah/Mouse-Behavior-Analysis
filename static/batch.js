...
function step(name,focus=false){
 if(!['videos','regions','results'].includes(name))name='videos';
 if(name==='regions'&&!entry()){notice('Add a recording first.',true);name='videos';}
 for(const n of ['videos','regions','results'])$('#batch-'+n).hidden=n!==name;
 ...
 history.replaceState(null,'','#'+name);window.scrollTo(0,0);if(focus)$('#'+name+'-heading').focus({preventScroll:true});
}
...
function renderLists(){
 ...
 for(const name of ['batch-queue','review-queue']){
  ...
  draft.entries.forEach((e,i)=>{
   ...
   if(name==='batch-queue')span.append(el('small',e.video.split('/').pop()));
   const b=el('button',e.reviewed?'Edit':'Review');b.setAttribute('aria-label',`${e.reviewed?'Edit':'Review'} regions for ${e.id}`);
   b.onclick=()=>{current=i;loadEntry();step('regions',true);};row.append(span,b);
   if(name==='batch-queue'){
 ...
 $('#start-batch').disabled=busy||adding||!total||count!==total;
 $('#start-batch').textContent=busy?'Analysis in progress…':`Analyze ${total===1?'recording':total+' recordings'} →`;
 $('#analysis-help').textContent=busy?'Open Results to follow progress.':count===total&&total?'Ready. Tracking and scoring will run automatically.':'Confirm the regions in every recording to continue.';
...
$('#batch-recording').onchange=e=>{current=Number(e.target.value);loadEntry();};$('#review-regions').onclick=()=>{loadEntry();step('regions',true);};
$('#batch-recording-id').oninput=e=>{entry().id=e.target.value;changed();};$('#batch-target').onchange=e=>{entry().config.target_side=e.target.value;changed();};
...
$('#add-all').onclick=guard(async()=>{adding=true;renderLists();renderAvailable();try{for(const {working} of workspaceChoices())await addVideo(working.name);if(entry())loadEntry();notice('Recordings added. Review their regions next.');}finally{adding=false;renderLists();renderAvailable();}});
$('#batch-upload').onchange=guard(async e=>{
 ...
 try{
  ...
  $('#upload-status').textContent=`${added} recording${added===1?'':'s'} added. ${failures.length?failures.length+' could not be added.':'Review their regions next.'}`;
  if(failures.length)notice(failures.join(' · '),true);else notice('Videos added. Continue to Review regions.');
...
function renderJob(b){
 ...
 for(const e of completed){
  ...
  if(e.summary){
   ...
   const video=el('video');video.controls=true;video.preload='metadata';video.setAttribute('aria-label',`Annotated review for ${e.id}`);video.src=`/outputs/${e.run_id}/review.mp4`;c.append(video);
...
guard(async()=>{
 [draft,profile,available]=await Promise.all([api('/api/batch/draft'),api('/api/profile'),api('/api/videos')]);
