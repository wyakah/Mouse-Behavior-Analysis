...
This is a local development release. Distribution to other Macs requires a Developer ID signature and Apple notarization, plus review of third-party runtime/model redistribution terms. Tauri signing credentials are supplied by the release operator; never commit credentials. The public development installer is on [GitHub Releases](https://github.com/wyakah/Mouse-Behavior-Analysis/releases/tag/v0.1.6). The build script itself does not publish releases or configure automatic updates. Intel Mac bundles are not yet supported. The [Windows x64 installer](https://github.com/wyakah/Mouse-Behavior-Analysis/releases/tag/v0.1.6) is built and tested with `.github/workflows/windows.yml`; publication is gated on installed-runtime smoke tests for both assays.
...
Dispatch the **Windows installer** GitHub Actions workflow. It uses Python 3.12 with CPU PyTorch, verifies the pinned platform-neutral model archive, builds a per-user NSIS installer, tests both installed analysis pipelines using synthetic video, and checks native startup/quit. The download is unsigned. WebView2 is installed by its Microsoft bootstrapper if missing; that first-time step requires internet. Analysis uses bundled models locally.
