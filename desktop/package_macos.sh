...
hdiutil create -ov -format UDZO -volname 'Behavior Studio' -srcfolder "$staging" 'dist/Behavior-Studio-0.1.6-apple-silicon.dmg'
# Some macOS versions leave the newly created image attached after compression.
python3 - <<'PYIMAGE'
import pathlib, plistlib, subprocess
image=pathlib.Path('dist/Behavior-Studio-0.1.6-apple-silicon.dmg').resolve()
...
PYIMAGE
hdiutil verify 'dist/Behavior-Studio-0.1.6-apple-silicon.dmg'
(cd dist && shasum -a 256 'Behavior-Studio-0.1.6-apple-silicon.dmg' > 'Behavior-Studio-0.1.6-apple-silicon.dmg.sha256')
