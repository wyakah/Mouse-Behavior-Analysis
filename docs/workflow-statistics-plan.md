...
Choose test → Recordings & groups → Review regions → Results. Three Chamber is available; Stereotypy is a clearly unavailable future pipeline. Existing video/region/results deep links and saved drafts continue to work. Test identity is saved in each new batch and export, and unknown/unsupported test IDs are rejected server-side.
